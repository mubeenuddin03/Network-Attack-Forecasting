...
class ApiClient {
  ...
  async getHealth(): Promise<DashboardHealth> {
    ...
    const dashboardHealth: DashboardHealth = {
      ...health,
  ...
  async predict(features: WindowFeatures, threshold: number = 0.5): Promise<DashboardPrediction> {
    const start = performance.now();
    const request: PredictRequest = { features, threshold };
    ...
    const prediction: DashboardPrediction = {
      ...response,
  ...
  onPredictionChange(callback: (prediction: DashboardPrediction) => void): () => void {
    this.predictionCallbacks.add(callback);
    return () => this.predictionCallbacks.delete(callback);
  }

  onStatusChange(callback: (status: 'connected' | 'degraded' | 'offline') => void): () => void {
  ...
  private notifyPredictionCallbacks(prediction: DashboardPrediction) {
    this.predictionCallbacks.forEach(cb => cb(prediction));
  }

  startHealthPolling(interval: number = HEALTH_CHECK_INTERVAL) {
    if (this.healthCheckTimer) return;
    this.isPolling = true;
    this.getHealth().catch(() => {});
    this.healthCheckTimer = setInterval(() => {
      if (this.isPolling) this.getHealth().catch(() => {});
    }, interval);
  }
  ...
  startPredictionPolling(features: WindowFeatures, threshold: number, interval: number = PREDICTION_POLL_INTERVAL) {
    if (this.predictionPollTimer) return;
    this.isPolling = true;
    const poll = () => this.predict(features, threshold).catch(() => {});
    poll();
    this.predictionPollTimer = setInterval(poll, interval);
  }

  stopPredictionPolling() {
    if (this.predictionPollTimer) {
      clearInterval(this.predictionPollTimer);
...
export function isRealModel(mode: ModelMode): boolean {
  return mode === 'REAL_MODEL';
}

export function getModelModeLabel(mode: ModelMode): string {
  return mode === 'REAL_MODEL' ? 'Real Model' : 'Demo Mode';
}

export function getModelModeDescription(mode: ModelMode): string {
  return mode === 'REAL_MODEL'
    ? 'Using trained Logistic Regression model'
    : 'Using heuristic fallback (not a trained model)';
}